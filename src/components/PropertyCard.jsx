...
export default function PropertyCard({ id, image, title, price, extras, span }) {
    ...
    return (
        <Link href={`/property/${id}`} className={`relative ${colspan} h-[160px] sm:h-[200px] md:h-[300px] rounded-md overflow-hidden group`}>
            <Image src={image} alt={title} fill={true} className="left-0 z-10 top-0 object-cover transition-all duration-500 scale-100 group-hover:scale-150" />
